...
- [ ] Alex: `Solo` (9.5GB, plex RPG_ARCHIVE) has never been backed up to cru3 — needs a sync.
...
- [ ] Confirm READING → cru1 backup sync completed. Dry run was still crawling ~160k Kindle-collection files at session end; live `rsync -av --ignore-existing` was kicked off but not confirmed finished. Note: an earlier mis-run partially copied READING_0726 to **cru3** by mistake before being redirected to cru1 — left in place per the no-delete-from-CRU rule, harmless redundant copy, no cleanup needed.
...
- [ ] Alex: log cru2 ("TROVE WEBSITE - NOT BOOKS") and cru3 (RPG_ARCHIVE) formally in backup_drives.md. cru1 (READING) backup sync now underway as of 2026-08-05 (see new session section above) — no longer just "queued," needs confirmation once complete. (Carried forward from 07-31.)
