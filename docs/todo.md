...
- [ ] **Check result of the 7-hour stress-ng soak test in isolated LXC 150** (started 2026-07-08/09, tmux session on shardik) — the whole point was reproducing the MCE fault in isolation from any production workload. Check tmux output first (`tmux capture-pane -pt <session>`), then decide next step with Jordan.
- ⚠️ **Second, distinct MCE bank decoded 2026-07-08/09: bank 5 (execution unit), UC+PCC set, hit 2 CPUs simultaneously** — `journalctl -k -b -1`. Different bank than the 2026-07-06 bank 0 finding below. RAM continues to look ruled out (aslan/maturin DIMM audits both came back clean and were actually a documentation error, not a hardware fault). Points more toward CPU or motherboard VRM. Needs the stress-ng soak test result above before further diagnosis.
- ⚠️ **NEW — Machine Check Exception found 2026-07-06 ~19:37, unresolved.** `CPU 6: Machine Check: 0 Bank 0: baa0000000060135` (VAL/UC/EN/PCC all set — Processor Context Corrupted, which is why it forced a reboot) in the previous boot's error log. Genuine hardware-level fault, distinct from RAM (already stress-tested clean twice) and the earlier RTC/corosync issue. **Correction to the standing "PSU suspected" theory: shardik's PSU is already new (replaced this week) — a fresh PSU covers wall→12V stability but is a different component from the motherboard's own VRM (steps 12V down to CPU core voltage).** Remaining suspects are motherboard VRM degradation or the CPU itself, not the PSU. rasdaemon installed but wasn't running when this happened (no historical decode available); now running for next occurrence. Needs Jordan: CPU-specific stress test under sustained load + VRM behavior/temps under load (not just the idle BIOS screen) before anything else gets tried on shardik.
