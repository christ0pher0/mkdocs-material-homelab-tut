...
- [x] ~~Verify Homepage "Pis" section icons render correctly after a refresh~~ — ✅ confirmed 2026-07-27 via live screenshot: Home Assistant/OctoPrint/Pi-hole each show distinct correct icons, Batocera/Pi2/RetroPie/Argos correctly share the generic raspberry-pi icon. No broken icons.
- [x] ~~"Missing Widget Type: ollama" on the Homepage Ollama/Babar tile~~ — ✅ fixed 2026-07-27. Root cause: gethomepage has no native Ollama widget (confirmed against the full official widget list — only a years-old open feature request exists). Fixed by switching to the documented `customapi` widget type pointed at Ollama's own `/api/tags` endpoint, mapped to show "Models installed." Applied to both Homepage instances.
- [x] ~~Ollama LXC (102, babar) IP mis-ranged~~ — ✅ fixed 2026-07-27, same pattern as the 2026-07-21 rocky-rpm fix. Was on `192.168.1.169` (160-179 = peripherals/IoT range, wrong category for a container) via a floating DHCP lease, no reservation. Verified `.20` and the 20-25 range were genuinely free via `nmap -sn` (not just "didn't answer once" — `.20` specifically had documented provenance, previously rocky-rpm's slot before its own 2026-07-21 move to `.51`). Added a DHCP reservation for the LXC's MAC (`BC:24:11:84:AF:CB`) → `192.168.1.20`, rebooted the container to pick it up (confirmed via `pct exec ... ip addr`), updated git-ansible's `/etc/hosts`, and both Homepage configs' href/ping/widget URLs. Live end-to-end verified: `curl http://192.168.1.20:11434/api/tags` returns real model data, both dashboards match.
- [x] ~~Homepage instances (monitor-deb primary, backup-dietpi-deb backup) brought to full parity~~ — ✅ done 2026-07-27, per Chris's original ask (the two dashboards had drifted — primary has full live widgets throughout, backup was mostly plain bookmark tiles). Copied primary's `services.yaml` directly onto the backup host, piped host-to-host via git-ansible so none of the embedded credentials (Proxmox root/pam, Gitea, Portainer, Grafana, Zabbix, Sonarr/Radarr/Prowlarr/Lidarr/Mylar/Komga/Audiobookshelf/RomM/qBittorrent) got re-echoed into any terminal. Backup's old standalone "This Host" Gitea-backup tile and the "Asset Management" section added earlier tonight were overwritten in the process, but both are already covered by the primary's own structure (its "Gitea (Backup)" and "Snipe-IT" entries), so no real data lost — just reorganized.
  - **Real gotcha found while verifying:** the config edit + restart looked successful (file diff came back identical, service restarted clean) but the dashboard still rendered the old layout. Root cause: backup-dietpi-deb runs Homepage as a standalone Next.js build (`homepage.service` → `node .next/standalone/server.js`, no Docker on this host at all), which resolves its config path *relative to the standalone directory* — `/home/cos/homepage/.next/standalone/config/services.yaml`, a completely separate file from the source-tree config at `/home/cos/homepage/config/services.yaml` that every edit tonight (including the earlier Ollama/plow-rpm additions) had actually been landing in. Fixed by copying the corrected file into the real standalone path and restarting the service — confirmed visually this time, not just via file diff. **Any future edit to this host's Homepage config must target the `.next/standalone/config/` path, not the source tree.**
