...
- ⚠️ **Three real, pre-existing issues surfaced now that the automation can actually reach them (not caused by the vault fix, newly visible because of it):**
  ...
  - **plow-rpm package update failed for real** — dnf transaction conflict: `containers-common-5:5.8-1.el9.x86_64` and `redhat-release-9.6-0.1.el9.x86_64` both claim `/etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release`. Needs Jordan to resolve (likely `dnf remove containers-common` + reinstall, or repo priority fix) before plow-rpm can patch cleanly again.
...
- Also noticed in passing: "apache-deb" appeared in a PLAY RECAP — not a host recognized from anything touched this session. Worth a look, not urgent.
...
- [x] Add babar to `~/ansible_dev/inventory_auto` — **actually root-caused and permanently fixed 2026-07-26 (morning follow-up).** Last night's `onboard2.yml` run (below) worked, but babar vanished again by 07:07am — turned out `inventory_auto` is fully regenerated **every hour** by a root cron job (`generate_inventory.py`, `7 * * * *`), which silently wiped the manual edit. Root cause: the generator hardcodes the `[proxmox]` block (babar was never in the list) and classifies `[linux]` membership purely by hostname suffix (`-deb`/`-rpm`/etc.) — "babar" has no suffix, same as the other bare-named Proxmox nodes, so it was structurally excluded both ways, every single run. This explains the entire recurring "babar keeps falling out of inventory" saga going back to 2026-07-16 — never a one-off miss. **Fix:** rewrote `generate_inventory.py` to hardcode babar into `[proxmox]` (matching shardik/maturin/aslan/blaine) and added an `EXTRA_LINUX_HOSTS` exception list (same pattern as the existing `PI_HOSTS` special-case) to force it into `[linux]` too. Ran once, confirmed babar now in both groups (`grep -n babar inventory_auto` → lines 4 and 97), 41 hosts generated clean. This survives every future hourly run — no more manual re-adds needed.
